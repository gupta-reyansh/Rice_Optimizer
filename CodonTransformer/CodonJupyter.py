...
def dna_to_protein(dna_sequence):
  ...
  # Clean sequence: uppercase and remove spaces/newlines
  clean_seq = dna_sequence.upper().replace('\n', '').replace('\r', '')

  protein = []
  # Loop through the sequence in steps of 3 nucleotides
  for i in range(0, len(clean_seq) - 2, 3):
    codon = clean_seq[i : i + 3]
    # Get amino acid from table, default to '?' if codon is invalid
